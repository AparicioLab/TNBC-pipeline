...
###############################################################
# Run as source("/arpa/ns/o/oncoapop/Scripts/check_UCSC_BSD2.R")
# Run using iterate.sh
# ie nohup Rscript check_UCSC_BSD2.R &
###############################################################
#
...
# Check lastrow later on in the script
...
################################################################
# Run on sdf cluster
sumdir="/arpa/ns/o/oncoapop/Projects/TNBC/primer3"
...
timestamp <- Sys.time()
print(timestamp)
# This gets the local time, start next job after 30 min
endtime <- system("TZ=GMT+7 date | sed 's/GMT/PDT/'")
print(endtime)
print("################################################################")

# Run script TNBC-aws.sh after each run to renames output and upload to AWS S3
# Run script TNBC-aws-script.sh after whole job to transfer main script and output file to AWS S3
